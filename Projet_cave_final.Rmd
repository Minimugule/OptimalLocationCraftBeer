...
```{r,echo=FALSE}
#data_temp<-matrix(0,nrow=nrow(iris_rennes),ncol=nrow(iso420))
#for(j in 1:nrow(iso420)){
#  for (i in 1:nrow(iris_rennes)) {
#    data_temp[i,j]<-as.character(st_intersects(iris_rennes, iso420,sparse=F)[i,j])
#    data_temp[i,j]<-ifelse(data_temp[i,j]=="TRUE",iso420$Date.categ[j],0)
#  }
#} #cela nous donne les enseignes des supermarchés accessibles dans chaque IRIS, avec une colonne par supermarché dans nos données

#data_temp2<-matrix(0,nrow=nrow(data_temp),ncol=length(unique(iso420$Date.categ)))
#for(j in 1:length(unique(iso420$Date.categ))){
#  data_temp2[,j]<-apply(data_temp, 1, function(x) length(which(x==unique(iso420$Date.categ)[j])))
#} #regroupe le dataframe différent avec le nombre de supermarché de chaque enseigne par IRIS

#data_temp2<-as.data.frame(data_temp2)
#colnames(data_temp2)<-unique(iso420$Date.categ)
#pensez bien à changer le repertoire ici
#write.csv(data_temp2,"data/data_temp2")
```
...
```{r methode_rook, warning=FALSE, message=FALSE}
...
```{r calccul_voisin, warning=FALSE, message=FALSE}
...
```{r proche voisin, warning=FALSE, message=FALSE}
...
L'analyse confirme une autocorrélation spatiale positive significative dans la variable "couverture". Cela signifie que les zones ayant des valeurs de "couverture" similaires sont regroupées géographiquement, quelle que soit la matrice de poids utilisée. Ce résultat suggère que des processus ou des facteurs spatiaux pourraient influencer la distribution des valeurs de "couverture" dans la zone d'étude.
...
Pour l'implentation d'une nouvelle cave à bière, il y aurait deux solutions possibles :
   ...
   - Implenter une cave à bière à Beaulieu: Cette zone ne dispose actuellement d'aucune cave, malgré la présence d'une population jeune, qui est souvent friande de bière. L'avantage réside dans le fait qu'il n'y a pas de cave facilement accessible, ce qui signifie peu de concurrence. Cela offre un avantage par rapport aux caves du centre-ville, qui sont plus difficiles d'accès.
